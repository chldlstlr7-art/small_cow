...
import { Share2, Search, TrendingUp, RotateCcw, Wallet, Calculator, Github, Globe } from 'lucide-react';
...
function StockCalculator() {
  ...
  // [핵심] 검색 로직 제어용 Ref
  const blockSearchEffect = useRef(false); // 클릭으로 입력 시 검색창 팝업 방지
  ...
  useEffect(() => {
    ...
    if (qTicker) {
      ...
      blockSearchEffect.current = true; // 초기 로딩 시 팝업 방지
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
        ...
        // [수정] 클릭으로 선택된 경우 검색 로직 스킵
        if (blockSearchEffect.current) {
            blockSearchEffect.current = false; // 플래그 해제
            return; 
        }
  ...
  const handleSelectStock = (symbol: string, name: string) => {
      blockSearchEffect.current = true; // [중요] 락 걸기
      setTicker(symbol);
            ...
            {showDropdown && searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl overflow-hidden max-h-60 overflow-y-auto">
                    {searchResults.map((item, idx) => (
                        <button 
                            key={idx} 
                            // onMouseDown을 써야 onBlur보다 먼저 실행됨
